...
//内部参数来源于初始化时最后一个参数
float MotorPower::getMotorRealCurrent(const float current) const {
    const float real_current = current/Current_Conversion;
    return real_current;
}

//预测类型默认为关闭预测(指你要填入电机反馈电流，来获取反馈的功率)
//使用时请根据你填入的电流类型来填入对应的枚举
float MotorPower::update(float current ,float speed,const E_Predict_Status_Type Predict_status,const E_CalMotorPower_Negative_Status_Type Negative_Status) {
    const float product = current*speed;
    float power_sign = 1;
...
float MotorPower::limiter(float *desired_current,const float current_speed, const float motor_power_limit ) {
    ...
    const float desired_current_ = *desired_current;

    const float real_desired_current = std::abs(getMotorRealCurrent(desired_current_));
    const float real_current_speed = std::abs(current_speed);

    if (const float predicted_power = update(desired_current_, current_speed,E_enable_not_limit_predict);
       predicted_power <= motor_power_limit) {
        update(*desired_current,current_speed,E_enable_predict);
        return 1.0;
       }

    const float a = K4 * real_desired_current * real_desired_current;
    const float b = (K1 + K3 * real_current_speed) * real_desired_current;
    const float c = K0 + K2 * real_current_speed + K5 * real_current_speed * real_current_speed - motor_power_limit;
    ...
    const float k1 = (-b - std::sqrt(discriminant)) / (2 * a);
    const float k2 = (-b + std::sqrt(discriminant)) / (2 * a);
...
//使用时效果不好请自行更改
std::vector<float> power_allocation_by_error(std::vector<float>& motor_errors_vector, float total_power_limit,const float buffer_power_attenuation) {
    ...
    for (float& error : motor_errors_vector) {
        error = std::abs(error);
    }
    ...
    const float total_error = motor_errors_vector[0]+motor_errors_vector[1]+motor_errors_vector[2]+motor_errors_vector[3];

    if (total_error <= M_Too_Small_AllErrors) {
        float equal_share = total_power_limit / motor_errors_vector.size();
        return {equal_share, equal_share, equal_share, equal_share};
    }

    std::vector<float> motor_power_limits_vector(4);
    if(total_power_limit < M_Motor_ReservedPower_Border) {
        for (int i = 0; i < 4; ++i) {
            const float ratio = motor_errors_vector[i] / total_error;
            motor_power_limits_vector[i] = ratio * total_power_limit;
        }
    }else {
        for (int j = 0; j < 4; ++j) {
            const float ratio = motor_errors_vector[j] / (total_error);
...
void ChassisPowerManager::allocatePower(const float total_power_limit, const float buffer_power_attenuation) {
    std::vector<float> errors_vec(motor_errors_.begin(), motor_errors_.end());
    const std::vector<float> allocated_power = power_allocation_by_error(errors_vec, total_power_limit, buffer_power_attenuation);
...
float ChassisPowerManager::getTotalPredictNotLimitPower() const {
    float total = 0.0;
...
float ChassisPowerManager::getTotalPowerLimit() const {
    float total = 0.0;
    for (const auto* motor : motors_) {
...
float ChassisPowerManager::getTotalPredictPower() const {
    float total = 0.0;
    for (const auto* motor : motors_) {
...
//todo:未测试
std::vector<float> allocate_SW_power(const float total_power, const float servo_rate, const float servo_predict_want_power) {
    ...
    //[0]为舵的功率，[1]为轮的功率
    std::vector<float> servo_wheel_power_vector(2);
    float servo_max_allocate_power = total_power * servo_rate ;
...
float rotate_speed_allocation(const int16_t vx, const int16_t vy, const int16_t rotate, const float alpha) {
    //简易分配，还可以加入最大功率限制的因素,效果不好就简单改下
    //实际比赛(底盘功率限制随等级增加的情况下)建议把速度和等级(最大功率)联系起来
    const float translation = sqrt(vx*vx + vy*vy);
    float adjusted_rotate = std::abs(rotate) - alpha * translation;
...
float MovingAverageFilter::update(const float new_value) {
    sum -= buffer[index];
